      ...
      <nav class="navbar navbar-default">
        <div class="topbar">
          ...
          <div class="topbar-left">
            ...
            <span class="navbar-page-title"> 博客管理 - 评论列表 </span>
          </div>
					...
					<div class="col-lg-12">
						<div class="card">
							<div class="card-toolbar clearfix">
								<form class="pull-right " method="get" action="${ctx}/blog-comment.html">       
							...
							<div class="card-body">
								...
								<div class="table-responsive">
									<table class="table table-bordered">
										...
										<tbody>
											<c:forEach items="${pageInfo.list}" var="blogcom">
												<tr>
													<td style="width: 50px;"><label
														class="checkbox checkbox-primary"> <input
															type="checkbox" name="ids" id="ids" value="${blogcom.cid}"><span></span>
													</label></td>
													<td style="width: 100px;">${blogcom.cid}</td>
													<td>${blogcom.ucontent}</td>
													<td>${blogcom.blog.btitle}</td>
													<td class="text-center">${blogcom.user.uname}</td>
													<td class="text-center"><fmt:formatDate value="${blogcom.utime}" type="date" pattern="yyyy-MM-dd HH:mm"/></td>
													<td class="text-center">
														<div class="btn-group">
															<a class="btn btn-xs btn-default" href="" title="查看" onclick="btn_edit(${blogcom.cid})"
																data-toggle="modal" data-target="#myModal"><i class="mdi mdi-eye-outline"></i></a>
															<a class="btn btn-xs btn-default" href="" title="删除" onclick='deleteBlogComment(${blogcom.cid})'
								...
								<ul class="pagination">
									<li><a
										href="${ctx}/blog-comment.html?currpage=${pageInfo.pageNum-1}"
									...
									<c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
										...
										<c:if test="${page_Num != pageInfo.pageNum }">
											<li><a style="margin-top: 7px;"
												href="${ctx}/blog-comment.html?currpage=${ page_Num}">${ page_Num}</a>
											</li>
										</c:if>
									</c:forEach>
									<li><a
                   ...
                   <div class="modal-body">
					 ...
					 <div class="form-group">
	                  ...
	                  <div class="form-group">
	                    ...
	                    <div class="col-xs-10">
	                      <textarea class="form-control" id="editCommentContent" name="mcontent" readonly="readonly" value="" rows="6" placeholder=""></textarea>
	                    </div>
	                  </div>
				</form>
                   </div>
...
<script type="text/javascript">
      ...
      if(checkNum==0){
          ...
          return;
           ...
           $.ajax({
               type:"post",
               url:"${ctx}/deleteManyOfBlogComment",
     ...
     var btn_edit=function(cid){
     	//alert("进入方法");
     	$.get("${ctx}/getBlogCommentById",{cid:cid},function(data){
     		$("#edituserName").val(data.user.uname);
     		$("#editBlogTitle").val(data.blog.btitle);
     		$("#editBlogContent").val(data.blog.bcontent);
     		$("#editCommentContent").val(data.ucontent);
     	});
     }
     
     //删除单条数据
  	var deleteBlogComment = function(cid) {
  		//alert("进入方法")
  		if(confirm("确定要删除吗"))
  		{
  			$.ajax({
  	    		url:"${ctx}/deleteBlogComment",
  	            type:"get", 
  	            data:{cid:cid},
  	            success:function (data) {
  	         	   if(data.result>0){
