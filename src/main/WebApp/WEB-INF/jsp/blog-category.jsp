      ...
      <nav class="navbar navbar-default">
        <div class="topbar">
          ...
          <div class="topbar-left">
            ...
            <span class="navbar-page-title"> 博客管理 - 分类列表 </span>
          </div>
          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-toolbar clearfix">
                <form class="pull-right " method="get" action="${ctx}/blog-category.html">       
				                      
					<input type="text" id="findByname" name="tname"  value="" placeholder="分类名称"/>
              ...
              <div class="card-body">
                ...
                <div class="table-responsive">
                  <table class="table table-bordered">
                    ...
                    <tbody>
                    <c:forEach items="${pageInfo.list}" var="blogtype">
                    	<tr>
                        <td>
                          <label class="checkbox checkbox-primary">
                            <input type="checkbox" name="ids" id="ids"  value="${blogtype.tid}"><span></span>
                          </label>
                        </td>
                        <td>${blogtype.tid}</td>
                        <td>${blogtype.tname}</td>
                        <td><fmt:formatDate value="${blogtype.createTime}" type="date" pattern="yyyy-MM-dd HH:mm"/></td>                       
                        <td>
                          <div class="btn-group">
                            <a class="btn btn-xs btn-default" onclick="btn_edit(${blogtype.tid})" href="#!" title="编辑" data-toggle="modal" data-target="#myModalUpdate"><i class="mdi mdi-pencil"></i></a>
                            <a class="btn btn-xs btn-default" onclick='deleteBlogType(${blogtype.tid})' href="#!" title="删除" data-toggle="tooltip"><i class="mdi mdi-window-close"></i></a>
                          </div>
                        </td>
                      </tr>
                    </c:forEach>                                           
				...
				<ul class="pagination">
					<li><a
						href="${ctx}/blog-category.html?currpage=${pageInfo.pageNum-1}"
					...
					<c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
						...
						<c:if test="${page_Num != pageInfo.pageNum }">
							<li><a style="margin-top: 7px;"
								href="${ctx}/blog-category.html?currpage=${ page_Num}">${ page_Num}</a>
							</li>
						</c:if>
					</c:forEach>
					<li><a
                  ...
                  <div class="modal-dialog" role="document">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                    		<form class="form-horizontal" id="addBlogType" action="#" method="post" enctype="multipart/form-data" onsubmit="return false;">
								 <div class="form-group">
				                    <label class="col-xs-8" for="example-text-input">分类名称：</label>
				                    <div class="col-xs-8">
				                      <input class="form-control" type="text" id="tname" name="tname" placeholder="请输入分类名称">
				                    </div>
				                  </div>
				                  <!-- <div class="form-group">
				                    <label class="col-xs-8" for="example-textarea-input">分类描述：</label>
				                    <div class="col-xs-8">
				                      <textarea class="form-control" id="typeNotes" name="typeNotes" rows="6" placeholder="请输入分类描述"></textarea>
				                    </div>
				                  </div> -->
							</form>
                      </div>
                      <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary addBlogType">点击保存</button>
                      </div>
                    </div>
                  </div>
                </div>
		...
		<div class="modal fade" id="myModalUpdate" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                  <div class="modal-dialog" role="document">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                    		<form class="form-horizontal" id="updateBlogType" action="#" method="post" enctype="multipart/form-data" onsubmit="return false;">
								 <input type="hidden" name="tid" id="edittid" value="${empty blogType ? '':blogType.tid}" />
								 <div class="form-group">
				                    <label class="col-xs-8" for="example-text-input">分类名称：</label>
				                    <div class="col-xs-8">
				                      <input class="form-control" type="text" id="edittypeName" name="tname" value="${empty blogType ? '':blogType.tname}" placeholder="">
				                    </div>
				                  </div>
...
$(function(){
  ...
  //修改
    $('.edit_update').on('click', function () {
    	//alert("进入方法");
    	$.ajax({
    		url:"${ctx}/updateBlogType",
            type:"post", 
            data:$("#updateBlogType").serialize(),
            success:function (data) {
         	   if(data.result>0){
         		   alert("修改成功");
         		   $(".model").hide();
...
<script type="text/javascript">
    ...
    var btn_edit=function(tid){
    	//alert("进入方法");
    	$.get("${ctx}/getBlogTypeById",{tid:tid},function(data){
    		$("#edittid").val(data.tid);
    		$("#edittypeName").val(data.tname);
    	});
    }
     
  //删除单条数据
	var deleteBlogType = function(tid) {
		//alert("进入方法")
		if(confirm("确定要删除吗"))
		{
			$.ajax({
	    		url:"${ctx}/deleteBlogType",
	            type:"get", 
	            data:{tid:tid},
	            success:function (data) {
	         	   if(data.result>0){
